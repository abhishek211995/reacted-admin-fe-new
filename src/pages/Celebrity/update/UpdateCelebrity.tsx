...
const UpdateCelebrity = () => {
    ...
    try {
      ...
      if (social_media_links) {
        ...
        socialMediaMaster.forEach((s) => {
          if (s.status === 'Active') {
            const valueMap = keyValuePair.find(
              (m) => m.platform === s.platform_name
            );
            if (valueMap) {
  ...
  const getUserDetails = async () => {
    ...
    const platformsData = await axios.get(
      baseUrl + '/get_all_social_media_platforms'
    );

    Promise.all([celebDetails, platformsData]).then((responses) => {
      fetchSocialMediaPlatforms(
        platformsData,
        celebDetails?.data?.data[0]?.social_media_links
      );
        ...
        <Container maxWidth="lg">
          <>
            <Typography sx={{ mb: 3 }} variant="h4">
              Update {celeb && `${celeb?.first_name} ${celeb?.last_name}`}{' '}
              Profile Details
            </Typography>
            <Grid container spacing={2}>
              <Grid item lg={12} md={12} xs={12}>
                {!loader && celeb ? (
                  <UpdateCeleberityDetails
